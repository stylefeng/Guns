<template>
  ...
  >
    <timer-form v-model:form="form" ref="timerFormRef"></timer-form>
  </a-modal>
</template>

<script setup name="TimerAddEdit">
...
// 点击保存
const save = async () => {
  timerFormRef.value.$refs.formRef.validate().then(async valid => {
    if (valid) {
      // 修改加载框为正在加载
      loading.value = true;
      ...
      // 执行编辑或修改
      if (isUpdate.value) {
        result = SysTimerApi.edit(form.value);
      } else {
        result = SysTimerApi.add(form.value);
      }
      result
        .then(async result => {
          // 移除加载框
